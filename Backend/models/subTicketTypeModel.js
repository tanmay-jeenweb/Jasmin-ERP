...
const createSubTicketType = async (ticketTypeId, name, assignedTo, addedBy, deviceId) => {
    const jsonAssignedTo = JSON.stringify(Array.isArray(assignedTo) ? assignedTo : [assignedTo]);
    const query = `
        INSERT INTO sub_ticket_type_master (ticket_type_id, name, assigned_to, added_by, device_id)
        VALUES (?, ?, ?, ?, ?)
    `;
    const [result] = await db.execute(query, [ticketTypeId, name, jsonAssignedTo, addedBy, deviceId]);
    return result;
};
...
const updateSubTicketType = async (id, ticketTypeId, name, assignedTo) => {
    const jsonAssignedTo = JSON.stringify(Array.isArray(assignedTo) ? assignedTo : [assignedTo]);
    const query = `
        UPDATE sub_ticket_type_master
        SET ticket_type_id = ?, name = ?, assigned_to = ?
        WHERE id = ?
    `;
    const [result] = await db.execute(query, [ticketTypeId, name, jsonAssignedTo, id]);
    return result;
};
