...
import { getProductTypes, createProductType, updateProductType, deleteProductType } from "../../api/productTypeApi";
import DataTable from "../../components/DataTable";
import toast from "react-hot-toast";
import { usePermission } from "../../context/PermissionContext";

// ─── Product Type Modal (Handles both Create and Edit) ───────────────────────────────────
function ProductTypeModal({ isOpen, row, onClose, onSave, saving }) {
  const [productTypeName, setProductTypeName] = useState("");

  useEffect(() => {
    if (row) {
      setProductTypeName(row.product_type_name || "");
    } else {
      setProductTypeName("");
    }
  }, [row, isOpen]);
  ...
  const handleSubmit = (e) => {
    ...
    onSave(isEdit ? row.id : null, productTypeName.trim());
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: "20px 28px", borderBottom: "1px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between", background: "linear-gradient(135deg,#6804a1,#52037e)" }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: "#fff" }}>{isEdit ? "Edit Product Type" : "Create Product Type"}</h2>
            <p style={{ margin: "4px 0 0", fontSize: 13, color: "#d9e2ec" }}>{isEdit ? "Update product type name" : "Add a new product type to the system"}</p>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ padding: "24px 28px" }}>
            <div style={{ marginBottom: 8 }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#475569", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 }}>
                Product Type Name <span style={{ color: "#e11d48" }}>*</span>
              </label>
              <input
                type="text"
                ...
                required
                placeholder="e.g. Smart Watch, Smartphone"
          ...
          <div style={{ padding: "16px 28px", borderTop: "1px solid #f1f5f9", display: "flex", justifyContent: "flex-end", gap: 12, background: "#fafafa" }}>
            ...
            <button
              type="submit"
              disabled={saving || !productTypeName.trim()}
              ...
              {saving ? "Saving…" : isEdit ? "Save Changes" : "Create Product Type"}
            </button>
          </div>
        </form>
      </div>
...
export default function ProductTypeMaster() {
  ...
  const loadProductTypes = async () => {
    ...
    try {
      ...
      setError("Unable to load product types. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (id, productTypeName) => {
    setSaving(true);
    try {
      if (id) {
        // Edit Mode
        ...
        // Create Mode
        await createProductType({ productTypeName });
      ...
      toast.error(err?.response?.data?.message || "Unable to save product type. Please try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message || "Unable to delete product type.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const columns = useMemo(() => {
    const cols = [
      { key: "id", label: "ID", minWidth: "80px" },
      {
        ...
        render: (row) => <span style={{ fontWeight: 700, color: "#0f172a" }}>{row.product_type_name}</span>
      }
    ];

    const canUpdate = hasPermission("product_type_master", "update");
    const canDelete = hasPermission("product_type_master", "delete");
    ...
    <div style={{ display: "flex", flexDirection: "column", flex: 1, background: "#f8fafc", fontFamily: "'Inter',sans-serif" }}>
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", width: "100%", margin: "0 auto", padding: "32px 30px" }}>
        ...
        <DataTable
          ...
          actionButton={
            hasPermission("product_type_master", "write") ? (
              <button
                onClick={() => {
                  setSelectedRow(null);
                  setIsModalOpen(true);
