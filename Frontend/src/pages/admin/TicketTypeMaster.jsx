...
// ─── Ticket Type Modal (Handles both Create and Edit) ───────────────────────────
function TicketTypeModal({ isOpen, row, onClose, onSave, saving }) {
  ...
  return (
    <div className="fixed inset-0 z-[1000] bg-slate-900/55 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-[18px] w-full max-w-[500px] mx-auto shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        {/* Modal Header */}
        <div className="px-7 py-5 border-b border-slate-100 flex items-center justify-between bg-gradient-to-br from-indigo-600 to-indigo-700">
          <div>
            ...
            <p className="mt-1 text-[13px] text-indigo-100">
              {isEdit ? "Update ticket type details" : "Add a new ticket classification category"}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit}>
          <div className="px-7 py-6">
            <div className="mb-2">
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
                Ticket Type Name <span className="text-rose-600">*</span>
              </label>
              <input
                type="text"
                value={name}
                ...
                required
                autoFocus
                placeholder="e.g. Hardware Issue, Billing Query, Network Error"
          ...
          <div className="px-7 py-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50">
            ...
            >
              {saving ? "Saving…" : isEdit ? "Save Changes" : "Create Ticket Type"}
            </button>
          </div>
        </form>
      </div>
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function TicketTypeMaster() {
  ...
  const handleSave = async (id, name) => {
    ...
    try {
      if (id) {
        await updateTicketType(id, { name });
        toast.success("Ticket type updated successfully");
      } else {
        await createTicketType({ name });
      ...
      toast.error(err?.response?.data?.message || "Unable to save ticket type. Please try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message || "Unable to delete ticket type.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const columns = useMemo(() => {
    // Only display ID, Name, and Actions. Timestamps are deliberately omitted from the frontend.
    const cols = [
    ...
    const canUpdate = hasPermission("ticket_type_master", "update");
    const canDelete = hasPermission("ticket_type_master", "delete");
    ...
    <div className="flex flex-col flex-1 bg-slate-50 font-sans min-h-screen">
      ...
      <main className="flex-1 flex flex-col w-full mx-auto px-[30px] py-8">
        ...
        <DataTable
          ...
          actionButton={
            hasPermission("ticket_type_master", "write") ? (
              <button
                onClick={() => {
                  setSelectedRow(null);
                  setIsModalOpen(true);
