...
import { getLandingTypes, createLandingType, updateLandingType, deleteLandingType } from "../../api/landingTypeApi";
import DataTable from "../../components/DataTable";
import toast from "react-hot-toast";
import { usePermission } from "../../context/PermissionContext";

// ─── Landing Type Modal (Handles both Create and Edit) ───────────────────────────
function LandingTypeModal({ isOpen, row, onClose, onSave, saving }) {
    ...
    <div className="fixed inset-0 z-[1000] bg-slate-900/55 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-[18px] w-full max-w-[500px] mx-auto shadow-2xl overflow-hidden">
        ...
        <div className="px-7 py-5 border-b border-slate-100 flex items-center justify-between bg-gradient-to-br from-indigo-600 to-indigo-700">
          <div>
            <h2 className="m-0 text-lg font-bold text-white">{isEdit ? "Edit Landing Type" : "Create Landing Type"}</h2>
            <p className="mt-1 text-[13px] text-indigo-100">{isEdit ? "Update landing type name" : "Add a new landing type to the system"}</p>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="px-7 py-6">
            <div className="mb-5">
              <label className="block text-xs font-bold text-slate-650 uppercase tracking-wider mb-2">
                Landing Type Name <span className="text-rose-650">*</span>
              </label>
              <input
                type="text"
                value={name}
                ...
                required
                placeholder="e.g. GST DP, net LANDING, MANAGER LANDING"
          ...
          <div className="px-7 py-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50">
            ...
            <button
              ...
              {saving ? "Saving…" : isEdit ? "Save Changes" : "Create Landing Type"}
            </button>
          </div>
        </form>
      </div>
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function LandingTypeMaster() {
  ...
  const loadLandingTypes = async () => {
    setLoading(true);
    setError("");
    try {
      const response = await getLandingTypes();
      setLandingTypes(response.data.data || []);
    } catch (err) {
      console.error("Failed to load landing types", err);
      setError("Unable to load landing types. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (id, name, live) => {
    ...
    try {
      if (id) {
        // Edit Mode
        await updateLandingType(id, { name, live });
        toast.success("Landing type updated successfully");
      } else {
        // Create Mode
        await createLandingType({ name, live });
        toast.success("Landing type created successfully");
      }
      setIsModalOpen(false);
      setSelectedRow(null);
      await loadLandingTypes();
    } catch (err) {
      console.error("Failed to save landing type", err);
      toast.error(err?.response?.data?.message || "Unable to save landing type. Please try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this landing type?")) return;
    setSaving(true);
    try {
      await deleteLandingType(id);
      toast.success("Landing type deleted successfully");
      await loadLandingTypes();
    } catch (err) {
      console.error("Failed to delete landing type", err);
      toast.error(err?.response?.data?.message || "Unable to delete landing type.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const columns = useMemo(() => {
    const cols = [
      ...
      {
        key: "name", label: "Landing Type Name",
    ...
    const canUpdate = hasPermission("landing_type_master", "update");
    const canDelete = hasPermission("landing_type_master", "delete");
  ...
  return (
    <div className="flex flex-col flex-1 bg-slate-50 font-sans">
      ...
      <main className="flex-1 flex flex-col w-full mx-auto px-[30px] py-8">
        ...
        <DataTable
          tableId="landing_type_master"
          title="Landing Type Master"
          data={landingTypes}
          columns={columns}
          loading={loading}
          searchPlaceholder="Search landing types..."
          actionButton={
            hasPermission("landing_type_master", "write") ? (
              <button
                onClick={() => {
                  setSelectedRow(null);
                  setIsModalOpen(true);
